...
void reset_password()
{
    printf("-----------重置密码-----------\n");
    char cardID[10];
    printf("请输入要重置密码的卡号(5位数字):\n");

    if (scanf("%9s", cardID) != 1)
    {
        printf("卡号输入失败，请重新输入！\n");
        while (getchar() != '\n');  // 清除非法输入
        return;
    ...
    while (card_current != NULL) // 遍历链表
    {
        if (strcmp(card_current->cardID, cardID) == 0)
        {
            ...
            if (card_current->state == 0)  // state为0表示未激活状态
            {
                printf("该卡未激活，无法重置密码！\n");
                system("pause");
                system("cls");
            ...
            strcpy(card_current->code, new_password); // 更新卡片的密码
