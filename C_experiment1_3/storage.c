...
static const char *text_field(const char *value)    // 将空字符串转换为"-"以便保存到文件中 static函数表示该函数只能在当前文件中使用
{  
...
{
    size_t len = strlen(line);            // size_t是无符号整数类型，适用于表示对象的大小，strlen函数用于计算字符串的长度
...
{
    int count_fields = 0;                   // 统计分割出的字段数量
    char *start = line;                     // 从行的开始位置开始分割字段 *start是一个指针，指向当前字段的起始位置
    char *pos;                              // 用于查找分隔符的位置
    ...
    while (count_fields < max_fields)       // 循环直到达到最大字段数量
    {
        ...
        *pos = '\0';                        // 将分隔符替换为字符串结束符，完成当前字段的分割                   
        start = pos + 1;                    // 更新start指针，指向下一个字段的起始位置
    }

    return count_fields;                    // 返回实际分割出的字段数量
}
...
{
    FILE *file = fopen(DATA_FILE, "r");  // 以只读模式打开数据文件，文件不存在就创建
    char line[1028];                     // 用于存储从文件中读取的一行文本
    char *fields[10];                    // 用于存储分割后的字段，最多支持10个字段
    int field_count;
    int i;

    initialize_data();     // 初始化数据的目的是为了在加载数据失败时能够恢复到一个干净的状态，避免程序中存在残留的旧数据导致错误的行为

    if (file == NULL)      // 如果文件无法打开，可能是因为文件不存在或者没有权限访问，此时直接返回0表示加载失败
    {
        return 0;
    }
    ...
    // 只有当这些验证都通过时，才认为加载数据的第一步成功了，count变量也被正确解析了，否则就关闭文件并初始化数据后返回0
    ...
    for (i = 0; i < count; i++)
    {
        ...
        // 如果验证通过了，就将分割后的字段值赋值给对应的cards数组元素的成员变量
        strcpy(cards[i].cardID, fields[1]);   
        strcpy(cards[i].code, fields[2]);
        strcpy(cards[i].last_time, fields[7]);
        restore_text_field(cards[i].last_time);  // 将last_time字段中的"-"转换回空字符串，以便在程序中使用
        ...
        restore_text_field(billings[i].last_time);   // 将last_time字段中的"-"转换回空字符串，以便在程序中使用
        ...
        restore_text_field(logins[i].login_time);  // 将login_time字段中的"-"转换回空字符串，以便在程序中使用
        ...
        restore_text_field(settles[i].settle_time);   // 将settle_time字段中的"-"转换回空字符串，以便在程序中使用
    }
    ...
    // 读取记录数量行，格式应该是"RECORD_COUNT|recordCount"，其中recordCount是当前记录的数量
    field_count = split_line(line, fields, 2);  
...
int save_data(void)         // 将程序中的数据保存到文件中，返回1表示成功，0表示失败
{
    ...
    // 以写入模式打开数据文件，如果文件不存在则创建，如果文件存在则覆盖原有内容
    // 每次写入都会覆盖原有内容，所以每次保存数据都会从头开始写入所有的数据，包括卡片信息、账单信息、登录信息、结算信息和消费记录，
    // 确保文件中的数据始终与程序中的数据保持一致。确保数据的更新
